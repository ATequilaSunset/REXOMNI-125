...
TASK_CONFIGS: Dict[TaskType, TaskConfig] = {
    ...
    TaskType.MOT_DETECTION: TaskConfig(
        name="MOT Detection",
        prompt_template=(
            "You are an expert in Multi-Object Tracking. "
            "Detect every single instance of {categories} in the image, regardless of their size. "
            "First, ensure all prominent and normal-sized objects are detected accurately. "
            "Then, pay extra attention to find any tiny, distant, blurred, or heavily occluded objects. "
            # [新增] 避免大块误识别的指令：
            "Do not merge multiple objects into one large box. Detect each object individually and tightly. "
            "Avoid outputting boxes that encompass the entire background. "
            "Output the bounding box coordinates in [x0, y0, x1, y1] format."
            "Detect all visible cars. If there are no cars, output nothing."
        ),
        description="Specialized detection for MOT, balancing normal and small object recall",
        output_format="boxes",
        requires_categories=True,
    ),
}
