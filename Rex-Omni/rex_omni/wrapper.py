...
class RexOmniWrapper:
    ...
    def _generate_transformers(self, messages: List[Dict]) -> Tuple[str, Dict]:
        ...
        # [Fix Warning] Only add sampling parameters if do_sample is True
        # 只有在开启采样时才传入 temperature/top_p/top_k，避免 transformers 报警告
        if do_sample:
    ...
    def _generate_transformers_batch(
        ...
        """Generate using Transformers model for batch processing with confidence scores"""
        ...
        # Generate for entire batch
        with torch.no_grad():
            # 现在 outputs 是一个包含 sequences 和 scores 的对象
            outputs = self.model.generate(**inputs, **generation_kwargs)

        generation_time = time.time() - generation_start

        # 提取生成的序列和分数
        generated_sequences = outputs.sequences  # shape: (batch_size, total_length)
        step_scores = outputs.scores  # tuple，每个元素 shape: (batch_size, vocab_size)

        # 将每一步的 logits 转换为概率
        import torch.nn.functional as F
        token_probabilities = [F.softmax(score, dim=-1) for score in step_scores]
        
        # 计算每个生成 token 的置信度（实际被选中的 token 的概率）
        batch_confidences = []
        prompt_length = inputs.input_ids.shape[1]
        
        for batch_idx in range(len(batch_messages)):
            # 获取这个样本生成的 token（跳过 prompt 部分）
            generated_tokens = generated_sequences[batch_idx, prompt_length:]
            # 获取每个生成 token 的置信度
            token_confidences = []
            for step_idx, step_probs in enumerate(token_probabilities):
                if step_idx < len(generated_tokens):
                    # step_probs[batch_idx] 是当前样本的概率分布
                    # generated_tokens[step_idx] 是实际选择的 token id
                    confidence = step_probs[batch_idx, generated_tokens[step_idx]].item()
                    token_confidences.append(confidence)
            batch_confidences.append(token_confidences)

        # 解码 batch results
        generated_ids_trimmed = [
            out_ids[len(in_ids):]
            for in_ids, out_ids in zip(inputs.input_ids, generated_sequences)
        ]
        ...
        for i, output_ids in enumerate(generated_ids_trimmed):
            ...
            # 计算平均置信度（仅针对生成的 token）
            avg_confidence = (
                sum(batch_confidences[i]) / len(batch_confidences[i])
                if batch_confidences[i] else 0.0
            )
            ...
            generation_info = {
                ...
                "tokens_per_second": tokens_per_second,
                # 新增置信度信息
                "token_confidences": batch_confidences[i],  # 每个 token 的置信度列表
                "average_confidence": avg_confidence,        # 平均置信度
            }
            batch_generation_info.append(generation_info)
