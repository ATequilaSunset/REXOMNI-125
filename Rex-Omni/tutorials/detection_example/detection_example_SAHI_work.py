...
def calculate_smart_rois(tracked_objects, img_w, img_h, margin=200, max_area_ratio=0.6):
    ...
    for group in groups:
        ...
        # 校验：如果区域太小（比如误检），或者太大（不如直接全图扫），则忽略
        roi_w = max_x - min_x
        roi_h = max_y - min_y
...
def parse_args():
    ...
    parser.add_argument("--image_folder", type=str, required=True, help="输入图片文件夹路径")
...
def main():
    ...
    SAVE_RESULTS_DIR = os.path.join(results_root, f"result{suffix}")
    os.makedirs(SAVE_RESULTS_DIR, exist_ok=True)
    # 可视化输出路径
    VIS_OUTPUT_DIR = os.path.join(output_root, f"vis{suffix}")
